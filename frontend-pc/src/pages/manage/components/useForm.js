...
const Demo = (props) => {
    ...
    if (props.editFormVisible && !show) {
        objData = props.defaultValue && props.defaultValue.schemas || {};
        setShow(o => !o)
    } else if (props.editFormVisible && show) {
    ...
    const goToFrPlayground = (record) => {
        window.open(`/playground/${props.defaultValue && props.defaultValue._id}`);
    ...
    return (
        <div>
            ...
            >
                ...
                <Button type="primary" onClick={goToFrPlayground} style={{ marginBottom: 12, position: 'absolute', bottom: 0 }}>
                    访问链接
                </Button>
            </Modal>
        </div>
